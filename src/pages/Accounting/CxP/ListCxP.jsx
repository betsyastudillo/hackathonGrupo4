...
import { getAccountsPayable } from '@/slices/thunks';

export const ListCxP = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      const cxp = await dispatch(getAccountsPayable());
      console.log('llega en lista cxp', cxp);
  ...
  const filteredAdvertising = advertising.filter(adver => {
    ...
    return (
      (adver?.descripcion || '')
        .toLowerCase()
        .includes(_searchingAdvertising.toLowerCase()) ||
  ...
  function _itemCount() {
    const _count =
      filteredAdvertising.length > itemsPerPage
        ? itemsPerPage
        : filteredAdvertising.length;
    return (
      <div className="pagination-wrap hstack gap-2">
        <span>
          {' '}
          Mostrando {_count} de {filteredAdvertising.length}
        </span>
      </div>
    );
  }
  ...
  function _tableBody() {
    return (
      <tbody className="list form-check-all">
        {accountsPayable?.map(cp => (
          <tr key={cp.codEmpresa}>
            <td>{cp.codEmpresa}</td>
            <td>{cp.nomEmpresa}</td>
            <td>{cp.nitEmpresa}</td>
            <td>{cp.nomBanco}</td>
            ...
            <Card className="bodyTable">
              <CardHeader>
                <h4 className="card-title mb-0">
                  Lista de Cuentas por Pagar a Comercios
                </h4>
              </CardHeader>
              <CardBody>
                    ...
                    <div className="noresult">
                      <div className="text-center">
                        ...
                        <p className="text-muted mb-0">
                          No hemos encontrado Cuentas por Pagar que coincidan
                          con tu búsqueda.
                        </p>
                      </div>
                    </div>
