...
import { setActiveMenu, changeSidebarVisibility } from '../slices/thunks';


const _sideBarChanging = (sidebarVisibilitytype) => {
...
const Navdata = () => {
  ...
  useEffect(() => {
    if (menus.length > 0) MenusMap();
  }, [menus]);


  const handleClick = ( menu) => {
    
    // Si no hay menú activo o es un menú diferente, actualizar
    if (menu.urlReact.length === 0 && menu.submenus.length > 0) {
      dispatch(changeSidebarVisibility('show'));
      _sideBarChanging(sidebarVisibilitytype);
      dispatch(setActiveMenu(menu));
    } else {
      dispatch(setActiveMenu(null));
      navigate(menu.urlReact);
    }
  };
  ...
  const MenusMap = () => {
    setMenuItems(
      menus.map((menu) => ({
        ...
        subItems: menu.submenus ? menu.submenus.map((submenu) => ({
          id: `submenu-${submenu.codItem}`,
          label: submenu.nomItem,
          link: submenu.urlReact,
          parentId: `menu-${menu.id}`,
        })) : [],
      }))
    );
  };
...
//   useEffect(() => {
//     document.body.classList.remove('twocolumn-panel');
//     if (iscurrentState !== 'Inicio') {
//       setIsInicio(false);
//     }
//     if (iscurrentState !== 'Pagos') {
//       setIsPagos(false);
//     }
//     if (iscurrentState !== 'Ventas') {
//       setIsVentas(false);
//     }
...
//     // {
//     //   id: 'pago-qr',
//     //   label: 'Pago con QR (App)',
//     //   icon: 'mdi mdi-qrcode-plus',
//     //   link: '/pago-con-qr',
//     //   click: function (e) {
//     //     e.preventDefault();
//     //     setIscurrentState('Pago con QR (App)');
//     //   },
//     // },
...
//       icon: 'pagos',
//       link: '/#',
//       stateVariables: isPagos,
//       click: function (e) {
//         e.preventDefault();
//         setIsPagos(!isPagos);
//         setIscurrentState('Pagos');
//         updateIconSidebar(e);
//       },
//       subItems: [
//         {
...
//       icon: 'ventas',
//       link: '/#',
//       stateVariables: isVentas,
//       click: function (e) {
//         e.preventDefault();
//         setIsVentas(!isVentas);
//         setIscurrentState('Ventas');
//         updateIconSidebar(e);
//       },
//       subItems: [
//         {
