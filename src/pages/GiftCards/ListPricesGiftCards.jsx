...
import { getListPricesGiftCards } from '../../slices/payments/giftCards/thunk';

export const ListPricesGiftCards = () => {
  ...
  const { listPricesGiftCards } = useSelector(state => state.GiftCards);
  ...
  useEffect(() => {
    ...
    dispatch(getListPricesGiftCards(data));
  }, []);
  ...
  const priceCard = async () => {
    try {
      const data = {
        id: currentCard.id,
        codGiftCardKupi: parseInt(codGrupo),
        valor: parseInt(currentCard.valor),
      };
      console.log('va al ws', data);
      if (isEditMode) {
        console.log('va en la data edit', data);
        // dispatch(updatePriceKupiKard(data));
      } else {
        console.log('va en la data create', data);
  ...
  function _tableBody() {
    return (
      <tbody className="list form-check-all">
        {listPricesGiftCards?.map((card, index) => (
          <tr key={index}>
            <td>{card.codGrupo}</td>
            <td>{card.nomPaquete}</td>
                      ...
                      <div className="noresult mb-5">
                        <div className="text-center">
                          ...
                          <p className="text-muted mb-0">
                            No hemos encontrado lista de precios para esta
                            GiftCard.
                          </p>
                        </div>
                      </div>
